...
X_class2 = np.matrix([x_train[i] for i in range(len(x_train)) if y_train[i]==2])
Y_class2 = np.matrix([2 for i in range(X_class2.shape[0])]).reshape(X_class2.shape[0],1)
...
plt.scatter(np.array(X_class2[:,0]), np.array(X_class2[:,1]),color='green')
...
plt.scatter(np.array(nearest_neighbors_of_query_point[:,0]),np.array(nearest_neighbors_of_query_point[:,1]),marker='s',s=150,color='yellow',alpha=0.30)
plt.xlabel('sepal length')
plt.ylabel('sepal width')
plt.legend(['class 0','class 1','class 2'])
